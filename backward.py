...
class BackwardChaining:
    def __init__(self,rule,fact,goal,file_name):
        ...
        self.iteration = 0
        self.road=None
        self.output += "CÁC DỮ LIỆU ĐẦU VÀO\n"
        ...
        self.output += "THỰC HIỆN SUY DIỄN LÙI \n"
    ...
    def print_step(self, goal, indent, msg):#indent : dấu gạch ngang
        self.iteration += 1
        self.output += '  Bước '+ str(self.iteration) + ": %sGoal %s. " % (indent, goal) + msg + "\n"
    ...
    def print_data(self, rules, facts, goal):
        self.output += "****Các tập luật phục vụ suy diễn lùi:\n"
        for rule in rules:
            self.output += "+ R%i: %s\n" % (rules.index(rule) + 1, str(rule))
        self.output += "\n****Các triệu chứng người dùng gặp phải: \n    %s.\n\n" % ", ".join(facts)
        self.output += "****Các bệnh dự đoán người dùng mắc phải:\n    %s.\n\n" % goal

    def print_result(self, result): # part 3
        if result is not False:

            if len(self.road) == 0:
                self.output += "    Goal %s được chứng minh.\n" % self.goal
                self.output += "    Không có đường đi suy diễn.\n"
            else:
                self.output += "    Goal %s được chứng minh.\n" % self.goal
                self.output += "    Road: %s.\n" % (self.road)
        else:
            self.output += "    Goal %s không được chứng minh.\n" % self.goal

    def write_output(self, file_name):
        self.output_file_name = "BC_%s.txt" % file_name.replace("/", ".")
