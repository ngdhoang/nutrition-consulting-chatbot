...
class ForwardChaining:
    ...
    def __init__(self, rule,fact,goal, file_name):
        ...
        self.output += "THỰC HIỆN SUY DIỄN TIẾN \n"
    ...
    def print_results(self, result, road, goal,facts):
        ...
        if result:
            ...
            self.output += "     Kết quả là : %s .\n" %", ".join(facts)
            self.output += "     Đường đi suy diễn được là:%s"  %", ".join(road) #new

    def write_output(self, file_name):
        self.output_file_name = "FC_%s.txt" % file_name.replace("/", ".")
